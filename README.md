...
- 捨棄舊有「字數除法」的相對猜測，改為直接抓取詞句開始秒數（`vocalStartSec`）。
- **歌曲對齊核心設計**：Phase 1 採「lyrics-as-anchor」策略——歌詞文本為唯一正確的文字來源，AI 聆聽音訊時只負責找出「每句歌詞從哪一秒開始唱」，完全不自行辨識歌詞文字，從根源消除 AI 辨識失真導致定錨偏移的問題。
- Phase 2 以前 6–8 字定錨關鍵字進行文字比對，並針對前奏、間奏、無投影片標記等邊界情況各有明確處理規則。
